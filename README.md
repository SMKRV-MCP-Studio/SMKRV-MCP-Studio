...
**Minimum** runs the app with 1-2 database connections under light load. **Recommended** provides headroom for multiple connections and concurrent queries. The image runs L0 regex prompt-injection detection out of the box. The L1 ML layer (DeBERTa ONNX) is included but opt-in: set `STUDIO_PROMPT_GUARD_ML_DOWNLOAD=true` to fetch the model (~180 MB, downloaded on first boot into the data volume) and enable it. Add roughly 300 MB RAM when L1 is on.
...
| `STUDIO_AGENT_SERVICE_TOKEN` | Shared token for agent-mcp and backend communication |
...
| `STUDIO_EXTERNAL_HTTPS_PORT` | `443` | External HTTPS port for HTTP to HTTPS redirects |

The admin account is not set via environment variables. On first launch, open the web UI and create the admin username/password through the **Setup** screen.
...
| [ONNX Runtime](https://onnxruntime.ai) | MIT | ML inference for the L1 prompt guard layer (opt-in) |
| [Tokenizers](https://github.com/huggingface/tokenizers) | Apache-2.0 | Text tokenization for the L1 prompt guard layer (opt-in) |
...
SMKRV MCP Studio is an independent product and is not affiliated with, endorsed by, or sponsored by any of the third-party trademark owners referenced herein.

PostgreSQL® and Postgres® are trademarks of the PostgreSQL Community Association of Canada. MySQL® is a registered trademark of Oracle Corporation. Redis® is a registered trademark of Redis Ltd. Apache Cassandra™ and Apache™ are trademarks of the Apache Software Foundation in the United States and/or other countries. ClickHouse® is a registered trademark of ClickHouse, Inc. Snowflake® is a registered trademark of Snowflake, Inc. Google BigQuery™ and Google Cloud™ are trademarks of Google LLC. Microsoft SQL Server® is a trademark of Microsoft Corporation. Supabase™ is a trademark of Supabase, Inc. Greenplum® is a trademark of VMware, Inc. Docker® is a registered trademark of Docker, Inc. Claude™ is a trademark of Anthropic, PBC. GPT™ and OpenAI™ are trademarks of OpenAI, Inc. Cursor™ is a trademark of Anysphere, Inc.

All other trademarks, logos, and brand names used in this project are the property of their respective owners and are used for identification purposes only.
