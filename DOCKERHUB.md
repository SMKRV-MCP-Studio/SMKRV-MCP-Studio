...
> **Minimum** runs the app with 1-2 connections under light load. **Recommended** handles multiple connections and concurrent queries. The image runs L0 regex prompt-injection detection out of the box; the L1 ML layer (DeBERTa ONNX) is included but opt-in via `STUDIO_PROMPT_GUARD_ML_DOWNLOAD=true` (~180 MB model fetched on first boot, +~300 MB RAM). For 10+ connections with heavy concurrency: 4-8 CPU, 8 GB RAM, 20 GB disk.
...
| agent-mcp | 8090 | AI agent interface (45 tools) |
...
All third-party product names, logos, and brands are the property of their respective trademark owners and are used for identification purposes only. Use of these names does not imply endorsement, affiliation, or sponsorship. PostgreSQL® is a trademark of the PostgreSQL Community Association of Canada. MySQL® is a registered trademark of Oracle Corporation. Redis® is a registered trademark of Redis Ltd. Apache Cassandra™ is a trademark of the Apache Software Foundation. ClickHouse® is a registered trademark of ClickHouse, Inc. Snowflake® is a registered trademark of Snowflake, Inc. Google BigQuery™ is a trademark of Google LLC. Microsoft SQL Server® is a trademark of Microsoft Corporation. Docker® is a registered trademark of Docker, Inc.
