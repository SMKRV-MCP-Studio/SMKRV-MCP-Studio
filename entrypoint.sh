...
missing_vars=""
for var in STUDIO_ENCRYPTION_KEY STUDIO_JWT_SECRET STUDIO_AGENT_SERVICE_TOKEN REDIS_PASSWORD; do
...
# Pre-upgrade SQLite backup (safety net for Watchtower auto-updates)
if [ -f /app/data/studio.db ]; then
    cp /app/data/studio.db /app/data/studio.db.pre-upgrade.bak 2>/dev/null || true
    echo "SQLite backup created: studio.db.pre-upgrade.bak"
fi
