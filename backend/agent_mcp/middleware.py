...
def _get_client_ip(request: Request) -> str:
    ...
    Priority: CF-Connecting-IP > X-Forwarded-For first entry > request.client.host
    """
...
class AuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if path in _CREDENTIAL_PATHS:
            ...
            # Pass client_ip through scope state for the handler
        ...
        # Validate token
        token_info = await auth.validate_token(bearer_token, client_ip)
