...
from agent_mcp.middleware import _get_client_ip
...
async def handle_token_request(request: Request) -> JSONResponse:
    ...
    # Use real client IP (CF-Connecting-IP / X-Forwarded-For aware)
    client_ip = _get_client_ip(request)
    cf_country = request.headers.get("cf-ipcountry")
    client_country = cf_country.upper()[:2] if cf_country and cf_country not in ("XX", "T1") else None
