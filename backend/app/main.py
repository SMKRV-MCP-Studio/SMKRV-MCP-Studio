...
async def _apply_schema() -> None:
    ...
    _migrations = [
        ...
        # Jinja transform + global variables (migration 008)
        ("server_config", "global_variables", "JSON DEFAULT '{}'"),
        ("tools", "transform_template", "TEXT"),
        # Agent MCP fields allowlist (migration 013)
        ("server_config", "agent_mcp_fields_allowlist", "JSON DEFAULT '[]'"),
    ]
    async with engine.begin() as conn:
    ...
    # Safety net: catch any mapped column missing from the hand-maintained list
    # above (the app manages SQLite schema at runtime and does NOT run alembic,
    # so `create_all` alone never ALTERs an existing table). Reflects live columns
    # and adds any absent mapped column, so a forgotten inline entry can no longer
    # produce a hard "no such column" crash on an upgraded database.
    async with engine.begin() as conn:
        added = await conn.run_sync(_add_missing_mapped_columns)
    for entry in added:
        logger.warning(
            "Schema drift auto-repaired: added missing column %s "
            "(add it to _apply_schema's explicit list with the correct default)",
            entry,
        )
...
def _add_missing_mapped_columns(sync_conn) -> list[str]:
    """Add any mapped column absent from an existing table (sync, for run_sync).

    Columns are added nullable (with the model's server_default when it has one).
    Precise non-null defaults live in the explicit list in ``_apply_schema``;
    this is only a backstop against future drift, run after that list.
    """
    ...
    for table_name, table in Base.metadata.tables.items():
        if table_name not in existing_tables:
            continue  # create_all already built brand-new tables in full
        ...
        for col in table.columns:
            ...
            try:
                ...
                # Only replicate a literal server_default. func.now()-style and
                # other non-constant defaults are skipped (SQLite rejects a
                # non-constant ADD COLUMN default anyway) and the column is added
                # nullable; extracting `.arg` defensively avoids an AttributeError
                # that would otherwise escape and crash-loop startup.
                default_sql = _column_default_sql(col.server_default)
                if default_sql is not None:
                    ddl += f" DEFAULT {default_sql}"
                sync_conn.exec_driver_sql(ddl)
                added.append(f"{table_name}.{col.name}")
            except Exception as exc:  # noqa: BLE001 - log and continue
                logger.warning("Schema drift repair failed for %s.%s: %s",
                               table_name, col.name, exc)
    return added


def _column_default_sql(server_default) -> str | None:
    """Return a literal SQL default for a column, or None if not constant."""
    ...
    if isinstance(arg, str):
        return arg
    return None  # non-constant (e.g. func.now()) - add the column nullable


async def _initial_deploy() -> None:
    """Regenerate the MCP server on first boot, or when the shipped templates changed.

    /shared/generated is a persisted volume, so after an image update (Watchtower
    recreates the container but keeps the volume) it still holds code generated by
    the OLD templates. Deploying only when server.py is absent would leave those
    users on stale generated code — including any security fix in the templates.
    We therefore also redeploy when the current template fingerprint differs from
    the one recorded at the last deploy.
    """
    from app.database import async_session_factory
    from app.services.deployer import Deployer

    deployer = Deployer()
    if not deployer.is_generated_stale():
        return

    logger.info("Generating MCP server files (missing or templates changed)...")

    async with async_session_factory() as db:
        result = await deployer.deploy(db)
        if result.deployed:
            logger.info("Deploy: %d files generated", result.files_generated)
        else:
...
async def _sync_mcp_tokens_to_redis() -> None:
    """Re-sync active MCP bearer tokens to Redis on every startup.

    Ensures tokens survive Redis restarts (generated MCP server validates
    via Redis only).
    """
    from app.database import async_session_factory
    from app.services.mcp_token_sync import sync_all_mcp_tokens_to_redis

    try:
        async with async_session_factory() as db:
            count = await sync_all_mcp_tokens_to_redis(db)
            if count:
                logger.info(
                    "MCP bearer tokens synced to Redis on startup: %d",
                    count,
                )
    except Exception:
        logger.warning(
            "Failed to sync MCP tokens to Redis on startup", exc_info=True
        )
...
async def lifespan(app: FastAPI):
    ...
    await _apply_schema()
    try:
        await asyncio.wait_for(_initial_deploy(), timeout=120)
    except Exception:
        # Never crash-loop startup on a slow or failing (re)deploy. The generated
        # code may be stale until the next boot (fingerprint stays mismatched) or
        # a manual UI deploy, but the app still serves.
        logger.warning("Startup deploy did not complete", exc_info=True)
    await _sync_nginx_config()
    await _sync_agent_tokens_to_redis()
    await _sync_oauth_sessions_to_redis()
    await _sync_mcp_tokens_to_redis()
...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "Authorization", "X-Agent-Service-Token"],
)
