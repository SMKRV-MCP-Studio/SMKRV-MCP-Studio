...
# Module-level thread pool for regex validation (avoids per-request executor overhead)
_regex_executor = concurrent.futures.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="regex-validate",
)


def _active_studio_port() -> int:
    """Return the currently running Studio UI port from settings."""
    return settings.port


def _active_studio_ssl_port() -> int:
    """Return the currently running Studio SSL port from settings."""
    return settings.ssl_port
...
async def validate_pattern(body: ValidatePatternRequest) -> ValidatePatternResponse:
    ...
    if body.test_text:
        ...
        try:
            future = _regex_executor.submit(_run_finditer)
            matches = future.result(timeout=2.0)
        except concurrent.futures.TimeoutError:
            raise HTTPException(
                status_code=408,
                detail="Pattern matching timed out (possible ReDoS)",
            )
