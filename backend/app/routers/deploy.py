...
# Maximum concurrent WebSocket log subscribers (M-20)
_MAX_WS_SUBSCRIBERS = 10
...
async def deploy(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    Concurrent requests are serialized by the deploy lock.
    """
...
async def deploy_logs_ws(
    ...
    if len(_log_subscribers) >= _MAX_WS_SUBSCRIBERS:
        await websocket.close(1013, "Too many log subscribers")
        return
