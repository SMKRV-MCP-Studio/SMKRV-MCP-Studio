...
async def list_tools(
    ...
    if tags:
        # Filter by tags in Python (JSON array in SQLite doesn't support SQL filtering)
        # Fetch all matching tools without pagination, then filter and paginate
        tag_list = [t.strip() for t in tags.split(",")]
        all_result = await db.execute(
            query.options(selectinload(Tool.parameters)).order_by(Tool.name)
        )
        all_items = list(all_result.scalars().unique().all())
        items = [t for t in all_items if any(tag in (t.tags or []) for tag in tag_list)]
        total = len(items)
...
async def duplicate_tool(tool_id: str, db: AsyncSession = Depends(get_db)) -> Tool:
    ...
    # Ensure unique name
    while True:
        existing = await db.execute(select(Tool).where(Tool.name == new_name))
