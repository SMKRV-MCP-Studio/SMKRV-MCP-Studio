...
from app.schemas.connection import assert_connection_host_safe
...
async def execute_preview(
    ...
    # Re-validate host + extra_params targets right before connecting (DNS-rebind guard).
    assert_connection_host_safe(connection.host, connection.db_type, extra)
    ...
    try:
        ...
        if data.transform_template:
            try:
                ...
                # Run the transform off the event loop with a hard timeout so a
                # heavy template cannot block the whole backend for other requests.
                loop = asyncio.get_running_loop()
                transformed_result = await asyncio.wait_for(
