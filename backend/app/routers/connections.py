...
from app.schemas.connection import (
    ...
    ConnectionUpdate,
    assert_connection_host_safe,
)
from app.services.crypto import (
    decrypt,
    decrypt_sensitive_extra,
    encrypt,
    encrypt_sensitive_extra,
    mask_sensitive_extra,
)
...
async def check_connections_health(
    ...
    async def _check_one(conn: Connection) -> dict:
        ...
        # Re-validate host + extra_params targets right before connecting so a
        # DNS rebind after the connection was saved cannot reach a private address.
        assert_connection_host_safe(conn.host, conn.db_type, extra)
...
async def update_connection(
    ...
    # The diff's "new" value comes from the raw (unencrypted) request body, so
    # mask sensitive extra_params fields (e.g. BigQuery credentials_json) before
    # they are persisted to the audit log. Also mask the snapshot for safety.
    if "extra_params" in changes and isinstance(changes["extra_params"], dict):
        ep = changes["extra_params"]
        if isinstance(ep.get("old"), dict):
            ep["old"] = mask_sensitive_extra(ep["old"])
        if isinstance(ep.get("new"), dict):
            ep["new"] = mask_sensitive_extra(ep["new"])
    snapshot = dict(before)
    if isinstance(snapshot.get("extra_params"), dict):
        snapshot["extra_params"] = mask_sensitive_extra(snapshot["extra_params"])
    await record_change(
        db, entity_type="connection", entity_id=connection.id,
        entity_name=connection.name, action="update", snapshot=snapshot, changes=changes,
    )
    await db.commit()
    await db.refresh(connection)
    return connection
