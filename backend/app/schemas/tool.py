...
from app.schemas.constants import VALID_RETURN_TYPES
...
def _validate_return_type(v: str) -> str:
    if v not in VALID_RETURN_TYPES:
        raise ValueError(
            f"Invalid return_type '{v}'. Must be one of: {', '.join(sorted(VALID_RETURN_TYPES))}"
        )
    return v
...
class ToolCreate(BaseModel):
    ...
    def validate_return_type(cls, v: str) -> str:
        return _validate_return_type(v)
...
class ToolUpdate(BaseModel):
    ...
    @field_validator("return_type")
    @classmethod
    def validate_return_type(cls, v: str | None) -> str | None:
        if v is not None:
            return _validate_return_type(v)
        return v
