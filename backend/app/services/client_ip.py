...
_GEOIP_DB_PATH = settings.geoip_db_path
_MAXMIND_LICENSE_KEY = settings.maxmind_license_key
_GEOIP_SOURCE = settings.geoip_source
...
def _parse_trusted_networks() -> list:
    """Parse settings.trusted_proxies (CSV of CIDRs) into ip_network objects."""
    import ipaddress
...
def get_client_ip(request: Request) -> str:
    ...
    Client-settable headers (CF-Connecting-IP, X-Forwarded-For) are honored only
    when the immediate peer is a configured trusted proxy; otherwise they are
    ignored and the socket peer is used. Behind the bundled nginx, the real IP is
    computed by the real_ip module into X-Real-IP (== $remote_addr), which a
    client cannot influence through the proxy — so that is the trusted source.
    A direct (unproxied) request falls back to the socket peer.
    """
        ...
        if src == "maxmind":
            ...
            with tarfile.open(fileobj=io.BytesIO(data), mode="r:gz") as tf:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    tf.extract(mmdb_member, tmpdir, filter="data")
