...
# ---------------------------------------------------------------------------
# Redis helpers (persistent connection pool — not a new connection per call)
# ---------------------------------------------------------------------------
...
def _hash_secret(plain: str) -> str:
    """Hash a secret with bcrypt (cost=12)."""
    salt = bcrypt.gensalt(rounds=_BCRYPT_ROUNDS)
    return bcrypt.hashpw(plain.encode(), salt).decode()


def _verify_secret(plain: str, hashed: str) -> bool:
    """Verify a secret against a bcrypt hash (constant-time)."""
...
async def list_agent_tokens(
    ...
    count_result = await db.execute(
        select(func.count()).select_from(AgentToken)
    )
        ...
        for s in result.scalars().all():
            if _verify_secret(access_token, s.access_token_hash):
                ...
                try:
                    ...
                    await r.set(f"agent:oauth_idx:{access_token[:12]}", key, ex=3600)
                except Exception:
                    pass
                break
...
async def get_activity_stats() -> dict:
    ...
    try:
        ...
        success_rate = round(
            (success_count / total_calls * 100) if total_calls > 0 else 0.0, 1
        )
        return {
...
async def check_agent_rate_limit(
    token_prefix: str, max_per_minute: int = 120
) -> bool:
