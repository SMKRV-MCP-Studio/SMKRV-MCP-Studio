...
def _bounded_range(*args):
    """range() capped so a transform cannot spin on range(10**9) (CPU/OOM).

    len(range(...)) is O(1), so an oversized range is rejected before iterating.
    """
    r = range(*args)
    if len(r) > _MAX_RANGE:
        raise ValueError(
            f"range() exceeds the {_MAX_RANGE}-element limit allowed in transforms"
        )
    return r
...
def _build_environment() -> SandboxedEnvironment:
    ...
    env.filters["sum_attr"] = lambda items, attr: sum(
        (item.get(attr, 0) if isinstance(item, dict) else 0) for item in items
    )
    env.filters["map_attr"] = lambda items, attr: [
        (item.get(attr) if isinstance(item, dict) else None) for item in items
    ]
...
def apply_transform(
    ...
    # Stream the render and cap output incrementally so a template that produces
    # a huge string (e.g. a large `{% for %}` loop) is aborted at the size limit
    # instead of building the whole output in memory first.
    chunks: list[str] = []
    total = 0
    for chunk in tmpl.generate(
        rows=rows,
        vars=global_vars or {},
        params=params or {},
    ):
