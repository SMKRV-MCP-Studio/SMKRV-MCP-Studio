...
- **Tool return type hardening** - the tool return type is now restricted to a fixed set of values and can no longer influence the generated server code.
- **Seamless upgrades** - the client IP used for login, two-factor, and OAuth rate limiting is taken only from the trusted reverse proxy, so lockout protection holds on a plain `docker run` deployment as well as behind Cloudflare.
- **Agent access resilience** - unauthenticated requests to the Agent MCP endpoint can no longer force expensive repeated work; token checks run off the request loop and per-IP limits apply before validation.
- **Audit log** - connection changes no longer record cloud credentials in plaintext in the change history.
- **Query preview** - the transform step runs with CPU, memory, and time limits so a heavy template cannot stall the app.

### Fixed

- **Upgrades from earlier 0.9.x** - a database created by an older version is now brought up to date automatically on startup, instead of failing with a missing-column error after the image updates.
- **Server code stays current** - after an image update that changes code generation, the MCP server is regenerated automatically so fixes reach existing installs (previously it kept running the old generated code).
- **Redis password on upgrade** - if `REDIS_PASSWORD` is not set, one is generated and stored, so an existing install that predates this requirement keeps running instead of failing to start.
- **Tools with edge-case names** - a parameter named like a Python keyword, a query ending in a quoted identifier, or an unusual server name no longer silently drops a tool from the deployed server.
- **Copy buttons** - one-time token and secret copy buttons now work over plain HTTP with a fallback and clear feedback.
- **Unsaved changes** - leaving a builder page with unsaved edits now prompts before discarding, including in-app navigation.
- **Session expiry** - an expired session now returns you to the login screen instead of leaving a broken page.

### Changed

- Dependencies updated for current security patches (Vite, React Router, Redis client, PyJWT, cryptography).
- Documentation corrected for the single all-in-one image: quick-start commands, required environment variables, tool count, and troubleshooting for the shipped container.
