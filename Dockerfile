...
# L1 ML prompt guard (DeBERTa ONNX) is bundled-capable and opt-in. The runtime
# (onnxruntime/tokenizers/numpy) and downloader ship in this image; enable it with
# STUDIO_PROMPT_GUARD_ML_DOWNLOAD="true" and the model (~180 MB) is fetched on first
# boot into the persisted model dir below. Left off by default so an existing
# install is not surprised by a large download on auto-update.
ENV PYTHONPATH="/app" \
    STUDIO_PROMPT_GUARD_MODEL_DIR="/app/data/models/prompt-guard" \
    ...
    STUDIO_PROMPT_GUARD_ENABLED="true" \
    STUDIO_PROMPT_GUARD_ML_DOWNLOAD="false" \
